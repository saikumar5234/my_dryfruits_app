...
const Register = ({ onToggleAuth }) => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Registration successful - the AuthContext will handle the user state
    } catch (error) {
              ...
              >
                <form onSubmit={handleRegister} style={{ width: '100%', maxWidth: 340 }}>
                  ...
                  <Button
                    ...
                    disabled={loading}
                  >
                    {loading ? <Fade in={loading}><PersonAdd sx={{ animation: 'spin 1s linear infinite' }} /></Fade> : t('signup')}
                  </Button>
                  <Button
                    variant="outlined"
                    color="primary"
